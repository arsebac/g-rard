...
async function validateTransition(projectId: number, fromStatus: string, toStatus: string) {
  ...
  // If no transitions defined, everything is allowed
...
export default async function taskRoutes(app: FastifyInstance) {
  app.get("/api/projects/:projectId/tasks", { preHandler: [requireAuth, requireProjectViewer] }, async (req, reply) => {
    try {
      ...
      // Inject project key into each task
  ...
  app.patch("/api/tasks/:id", { preHandler: requireAuth }, async (req, reply) => {
    ...
    // Workflow validation if status changes
    if (rest.status && rest.status !== old.status) {
      const allowed = await validateTransition(old.projectId, old.status, rest.status);
      if (!allowed) {
        return reply.status(400).send({ error: `Transition not allowed from ${old.status} to ${rest.status}` });
      }
    }
  ...
  app.patch("/api/tasks/:id/move", { preHandler: requireAuth }, async (req, reply) => {
    ...
    if (body.data.status !== old.status) {
      ...
      if (!allowed) {
        return reply.status(400).send({ error: `Transition not allowed from ${old.status} to ${body.data.status}` });
      }
    }
  ...
  app.delete("/api/tasks/:id", { preHandler: requireAuth }, async (req, reply) => {
    ...
    if (!taskToDelete) return reply.status(404).send({ error: "Task not found" });
